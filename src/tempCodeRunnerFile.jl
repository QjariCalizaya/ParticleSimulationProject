...
ax = Axis3(
    ...
    ylabel = "y",
    zlabel = "z",
    title = "Simulación 3D de partículas"
)
...
# Panel de controles
Label(control_panel[1, 1:3], "Controles de simulación", fontsize = 20)
...
Label(
    control_panel[4, 3],
    lift(x -> "duración = $(round(x, digits = 1)) s", duration_slider.value)
)
...
energy_button = Button(
    control_panel[6, 1:3],
    label = "Guardar gráfica de error"
)

compare_button = Button(
    control_panel[7, 1:3],
    label = "Comparar Euler vs Verlet"
)

Label(control_panel[8, 1:3], "Partículas", fontsize = 18)
...
Label(time_panel[1, 1], "Tiempo de visualización", fontsize = 18)
...
time_label = Label(time_panel[1, 3], "t = 0.0 s")
...
function apply_preset!(preset_name)
    if preset_name == "Órbita gravitacional"
        method_menu.selection[] = "Verlet"
    ...
    elseif preset_name == "Euler inestable"
        method_menu.selection[] = "Euler"
    ...
    elseif preset_name == "Atracción eléctrica"
        method_menu.selection[] = "Verlet"
    ...
    elseif preset_name == "Repulsión eléctrica"
        method_menu.selection[] = "Verlet"
    ...
    elseif preset_name == "Sistema mixto"
        method_menu.selection[] = "Verlet"
...
function update_time_view!()
    ...
    time_label.text[] = "t = $(round(current_time, digits = 2)) s"
...
on(preset_button.clicks) do _
    apply_preset!(preset_menu.selection[])
end
